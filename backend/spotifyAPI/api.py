...
def search_for_artists_albums_ids(artists: dict) -> List[str]:
    ...
    for artist in tqdm(artists, desc="artists"):
        ...
        artist_albmum_ids = [artist_album["id"]
...
def search_for_track_features_by_artists(artist: dict):
    ...
    # i = 0
    for artist in tqdm(artists, desc="artists"):
        track_information, _ = search_for_albums_songs(
            artists[artist]["album_ids"], track_information
        )
    ...
    tracks_audio_featues, not_found_track_ids = search_for_tracks_audio_features(
        track_ids
    )
    ...
    for track in track_information:
        if track in not_found_track_ids:
            continue
        track_information[track] = merge_dicts(
            track_information[track], tracks_audio_featues[track]
        )
...
def search_for_tracks_audio_features(track_ids: List[str]) -> dict:
    ...
    for track_id in tqdm(track_ids, desc="tracks"):
        try:
            ...
            tracks_audio_featues[audio_features["id"]] = {
